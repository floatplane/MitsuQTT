...
// generated via
// cat test/native/test_template/mustache_specs/interpolation.json| jq -r '.tests[] | \
// "TEST_CASE(\"\(.name)\") { ArduinoJson::JsonDocument data; deserializeJson(data, \
// R\"(\(.data))\"); CHECK_MESSAGE(Template(R\"(\(.template))\").render(data) == \
// R\"(\(.expected))\", R\"(\(.desc))\"); }"' | pbcopy
